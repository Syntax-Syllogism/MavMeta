			...
			>
				<div class="rest-request-row">
					...
					<datalist id="rest-starter-paths">
						{#each starterPaths as p (p)}
							<option value={p}></option>
						{/each}
					</datalist>
					<button class="btn btn--primary" type="submit" disabled={isLoading || !path.trim()}>
		...
		{#if response}
			<section class="rest-response-section" aria-label="Response">
				...
				<div class="rest-response-meta">
					<span
						class="rest-status-badge {statusClass(response.status)}"
...
{#if showConfirm}
	<div class="modal-backdrop">
		<div class="modal" role="dialog" aria-modal="true" aria-label="Confirm mutating request">
			<form
				onsubmit={(e) => {
					e.preventDefault();
					void doSend();
			...
			>
				...
				{#if isProduction}
					<div class="warning-box">
						<p>
							This is a <strong>production org</strong>. Mutating requests may have unrecoverable
							side effects.
						</p>
					</div>
					<label>
