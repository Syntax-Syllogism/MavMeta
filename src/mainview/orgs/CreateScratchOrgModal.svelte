<script lang="ts">
  ...
  const devHubOrgs = $derived(
    orgs.filter((org) => org.environment === "developer"),
  );
  const otherOrgs = $derived(
    orgs.filter((org) => org.environment !== "developer"),
  );
  ...
  const currentStepIndex = $derived(
    stepLabels.findIndex((s) => s.step === currentStep),
  );
  ...
  async function startCreate() {
    ...
    try {
      ...
      createError = error instanceof Error ? error.message : "Failed to start scratch org creation.";
      isCreating = false;
    }
  }
      ...
      {#if currentStep === "devhub"}
        <div class="wizard-section">
          <p class="wizard-section-hint">
            Select the Dev Hub org to create your scratch org from.
          </p>
          ...
          {#if otherOrgs.length > 0}
            <p class="field-label other-orgs-label">Other Orgs <span class="badge-unknown">Dev Hub status unknown</span></p>
            <div class="org-picker">
            ...
            <div class="field-group">
              ...
              <div class="tag-input-row">
                <input
                  ...
                  autocomplete="off"
                  onkeydown={(e) => { if (e.key === "Enter") { e.preventDefault(); addSetting(); } }}
                />
              ...
              {#each settingEntries as entry (entry.group)}
                ...
                <div class="settings-group-card">
                          ...
                          <tr>
                            <td>
                              <input
                                ...
                                onchange={(e) => toggleSubKey(entry.group, subKey.name, e.currentTarget.checked)}
                              />
                            </td>
                            <td>{subKey.name}</td>
                          </tr>
                  ...
                  {:else if !curatedGroup}
                    <p class="settings-group-hint">Configure sub-keys in the JSON editor (Step 4).</p>
                  {/if}
                </div>
              {/each}
            </div>
