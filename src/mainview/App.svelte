<script lang="ts">
  ...
  let lastSavedListValidationErrors = $state<Array<{ metadataType: string; reason: string }>>([]);
  ...
  const metadataTypesCache = new Map<string, { types: MetadataTypeSummary[]; apiVersion?: string }>();
  const metadataComponentsCache = new Map<string, {
  ...
  const activeOrgLabel = $derived(
    activeOrg?.alias ?? activeOrg?.username ?? "No active org",
  );
  ...
  const shouldGroupMetadataComponents = $derived(
    selectedMetadataType?.inFolder === true,
  );
  ...
  const stagedItemGroups = $derived(
    buildStagedItemGroups(activeOrgStagedItems),
  );
  const sourceOrgUsername = $derived(
    deriveSingleSourceOrgUsername(activeOrgStagedItems),
  );
  const sourceOrgLabel = $derived(
    orgs.find((org) => org.username === sourceOrgUsername)?.alias ?? sourceOrgUsername ?? "Unknown source org",
  );
  const compareTargetOptions = $derived(
    listEligibleTargetOrgs(orgs, sourceOrgUsername),
  );
  ...
  const compareTargetInstanceUrl = $derived(
    selectedCrossTargetOrg?.instanceUrl,
  );
  ...
  const deployConfirmationPhrase = $derived(
    getDeployConfirmationPhrase(
      cartAction === "deploy"
        ? (compareTargetLabel === "Select target org" ? undefined : compareTargetLabel)
  ...
  $effect(() => {
    ...
    const nextStagedItems = stagedItems.filter((item) =>
      validOrgUsernames.has(item.orgUsername)
    );
  ...
  async function loadOrgs() {
    ...
    try {
      ...
      statusMessage = response.orgs.length
        ? "Org list loaded."
        : "No Salesforce orgs found yet.";
    } catch (error) {
  ...
  function selectMetadataType(xmlName: string) {
    if (selectedMetadataTypeXmlName === xmlName) {
      return;
    }

    selectedMetadataTypeXmlName = xmlName;
    clearMetadataComponentExplorer();
  }
  ...
  function confirmOrgSwitch() {
    ...
    stagedItems = stagedItems.filter(
      (item) => item.orgUsername !== activeOrg.username,
    );
  ...
  function toggleAllStagedItems(components: MetadataComponentSummary[], metadataType: string) {
    ...
    const componentIds = components.map((c) => toStagedItemId(activeOrg!.username, metadataType, c.fullName));
    ...
    if (allStaged) {
      ...
      const newStagedItems: StagedItem[] = components
        .filter((c) => !alreadyStagedIds.has(toStagedItemId(activeOrg!.username, metadataType, c.fullName)))
        .map((c) => ({
  ...
  async function loadSavedShoppingList(listId: string) {
    ...
    try {
      ...
      const foundItems = validation.found.map(({ item, component }) => ({
      ...
      lastSavedListValidationErrors = validation.erroredTypes;
      cartAction = undefined;
      cartStep = "list";
      deployTypedConfirmation = "";
      ...
      if (validation.erroredTypes.length) {
        const erroredCount = validation.erroredTypes.reduce(
          (count, next) => count + next.count,
          0,
        );
        summaryParts.push(`${erroredCount} could not be validated due to metadata-type lookup errors.`);
      }
  ...
  function renameSavedShoppingList(listId: string) {
    ...
    savedMetadataShoppingLists = savedMetadataShoppingLists.map((candidate) =>
      candidate.id === list.id
        ? { ...candidate, name: nextName, updatedAt: now }
        : candidate
    );
  ...
  async function validateSavedListItemsForActiveOrg(items: SavedMetadataShoppingListItem[]) {
    ...
    const found: Array<{ item: SavedMetadataShoppingListItem; component: MetadataComponentSummary }> = [];
    ...
    const results = await Promise.all(typeEntries.map(async ([metadataType, typeItems]) => {
      try {
        const response = await backendClient.listMetadataComponents({
          target: { username: activeOrg.username },
          metadataType,
        ...
        const foundForType: Array<{ item: SavedMetadataShoppingListItem; component: MetadataComponentSummary }> = [];
        ...
        console.error(
          `[saved-list-validation] ${metadataType} lookup failed`,
          error,
        );
        return {
          found: [] as Array<{ item: SavedMetadataShoppingListItem; component: MetadataComponentSummary }>,
  ...
  async function runDeleteFromCart() {
    ...
    try {
      const started = await backendClient.startDestructiveDeploy({
        target: { username: activeOrg.username },
        mode: runMode,
        components: activeOrgStagedItems.map((item): HitListComponentInput => ({
          metadataType: item.metadataType,
          fullName: item.fullName,
        })),
      });
  ...
  function toErrorMessage(error: unknown) {
    return error instanceof Error
      ? error.message
      : "MavMeta could not reach the backend process.";
  }
  ...
  function toMetadataComponentListErrorMessage(error: unknown) {
    ...
    if (
      ...
      message.includes("listMetadataComponents")
    ) {
  ...
  function extractDeployJobId(result: DestructiveDeployResult | CrossOrgDeployResult): string | undefined {
    const candidate = result.rawResult && typeof result.rawResult === "object"
...
<main class="app-shell" class:busy={isBusy} class:light-theme={!isDarkTheme}>
  ...
  <section class="workspace">
    ...
    <section
      class="content-grid"
      aria-label={`${selectedTool} workspace`}
    >
  ...
  {#if isOrgSwitchAlertOpen}
    <div class="modal-backdrop">
      <div class="modal warning-modal" role="dialog" aria-modal="true" aria-label="Confirm org switch">
        <div>
  ...
  <MetadataCartWizard
    ...
    isProductionLikeTarget={cartAction === "deploy" ? isCrossTargetProductionLike : isProductionLikeTarget}
