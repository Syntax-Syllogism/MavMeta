	...
	if (!Array.isArray(report.coverageGaps)) {
		...
		report.coverageGaps.forEach((gap, index) => {
			...
			if (!AREA_VALUES.has(gap.area)) {
				issues.push(
					`coverageGaps[${index}].area must be metadata-explorer|object-explorer|lwc-editor|rest-explorer|soql-explorer|cart.`,
				);
			}
	...
	if (!Array.isArray(report.errors)) {
		...
		report.errors.forEach((errorItem, index) => {
			...
			if (!AREA_VALUES.has(errorItem.area)) {
				issues.push(
					`errors[${index}].area must be metadata-explorer|object-explorer|lwc-editor|rest-explorer|soql-explorer|cart.`,
				);
			}
