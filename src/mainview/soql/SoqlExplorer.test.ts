...
describe("SoqlExplorer", () => {
	...
	beforeEach(() => {
		...
		mockedClient.soqlBulkStatus.mockResolvedValue({
			jobId: "750xx000000001AAA",
			state: "JobComplete",
		});
	...
	it("does not auto-validate before object selection", async () => {
		...
		expect(
			screen.getByRole("button", { name: "+ Add filter" }).getAttribute("disabled"),
		).not.toBeNull();
	...
	it("builds filter/order/limit query from builder controls", async () => {
		...
		await fireEvent.change(screen.getByLabelText("Filter operator 1"), {
			target: { value: "LIKE" },
		});
	...
	it("builds picklist IN filters from multi-select values", async () => {
		...
		await fireEvent.click(screen.getByRole("button", { name: "+ Add filter" }));
		await fireEvent.change(screen.getByLabelText("Filter field 1"), {
