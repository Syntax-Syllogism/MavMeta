...
describe("smoke-crawl crawler panel state classification", () => {
	...
	it("classifies cart compare outcomes", () => {
		expect(
			classifyCartCompareOutcome({
				hasResult: true,
				hasEmptyState: false,
	...
	it("targets the cart wizard next button with an exact accessible name", () => {
		...
		getCartNextButton(page as never);

		expect(calls).toEqual([["button", { name: "Next", exact: true }]]);
	});
});
