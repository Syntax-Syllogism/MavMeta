...
- `--areas <list>`: comma-separated areas to crawl.
  - Allowed: `metadata-explorer`, `object-explorer`, `lwc-editor`, `rest-explorer`, `soql-explorer`, `cart`
...
- Auto-stages up to three real components from Metadata Explorer.
- Opens the Metadata Cart wizard and drives list -> actions -> confirm.
- Stops the destructive Delete path at the confirm screen and captures evidence.
- Runs Compare with org end to end when a second org exists; skips cleanly with one org.
- Clears staged items and closes the drawer before ending the pass.
